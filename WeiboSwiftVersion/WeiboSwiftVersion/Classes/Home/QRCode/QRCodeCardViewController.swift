...
//  WeiboSwiftVersion
//
//  Created by paomoliu on 16/8/23.
...
class QRCodeCardViewController: UIViewController {
    ...
    private func createQrCodeImage() -> UIImage
    {
        ...
        //3.设置需要生成二维码的数据
        filter?.setValue("pamoliu".dataUsingEncoding(NSUTF8StringEncoding), forKey: "inputMessage")
    ...
    /**
     ...
     :returns: 生成好的图片
     */
    private func createNonInterpolatedUIImageFormCIImage(ciImage: CIImage, size: CGFloat) -> UIImage
    {
    ...
    private func createImage(bgImage: UIImage, iconImage: UIImage) -> UIImage
    {
        ...
        let x = (bgImage.size.width - width) * 0.5   //尽量不要用除法，乘法性能更好一点
